...
import asyncio
from utils.asr_deepgram import transcribe_audio
...
def generate_timestamps(audio_path):
    """
    Wrapper to call Deepgram ASR and return word-level timestamps
    in the same format Whisper used before.
    """
    try:
        words = asyncio.run(transcribe_audio(audio_path))
        return {"timestamps": words}  # ✅ Matches DB schema
    except Exception as e:
        logging.error(f"Timestamp generation failed: {e}")
        return {"timestamps": []}
